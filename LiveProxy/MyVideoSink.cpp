...
MyVideoSink* MyVideoSink::createNew(UsageEnvironment& env) {
  return new MyVideoSink(env);
}
*/
/**
Singleton constructor
*/
MyVideoSink::MyVideoSink(UsageEnvironment& env):
MediaSink(env) , m_ready(0)
{
	TRACE_INFO("Video sink constructor");
	//m_fStreamId = strDup(streamId);
	//m_format = strDup(format);
	...
	//m_decoder=new CVideoDecoder(m_format);
...
Boolean MyVideoSink::continuePlaying() 
{
	if (fSource == NULL) 
	{
...
void MyVideoSink::afterGettingFrame(void* clientData, unsigned frameSize, unsigned numTruncatedBytes,
				  struct timeval presentationTime, unsigned durationInMicroseconds) 
{
	TRACE_DEBUG("afterGettingFrame. FrameSize=%d",frameSize);
	MyVideoSink* sink = (MyVideoSink*)clientData;
...
void MyVideoSink::afterGettingFrame1(unsigned frameSize, struct timeval presentationTime) 
{
    int got_frame = 0; 
